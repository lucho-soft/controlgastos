...
const baseStyles = `
  <style>
    ...
    th { background:#e3f2fd; padding:10px; text-align:left; }
    ...
    .bar-h { height:100%; border-radius:999px; }
...
function computeSummary() {
  ...
  // Aplicar ajuste de gastos del depto a Gerardo
  if (gerardoId) aportes[gerardoId] += ajusteGerardo;

  // --- NUEVA LÓGICA CORRECTA ---
  let aportesEmilse = aportes[emilseId] || 0;
  ...
  const cantHermanos = hermanos.length;
  const equalShare = gastosParaHermanos / cantHermanos;

  // Promedio para gráfico de equidad
  let totalNet = 0;
  hermanos.forEach((h) => (totalNet += aportes[h.id]));
  ...
  const resumenHermanos = hermanos.map((h) => ({
    ...
    name: h.name,
    aport: aportes[h.id],
    saldoPozo: aportes[h.id] - equalShare,
    diffEquidad: aportes[h.id] - avgNet,
  }));
...
function renderPage({ title, content }) {
  return `
    ...
    <div class="navbar">
      ...
      <nav><a href="/">Hermanos</a> <a href="/admin">Admin</a></nav>
    </div>
...
app.get("/", (req, res) => {
  ...
  const diffRows = s.resumenHermanos
    .map((r) => {
      ...
      const txt =
        r.diffEquidad > 0 ? "Puso de más" :
        r.diffEquidad < 0 ? "Puso de menos" : "En línea";
      return `
        <tr>
          <td>${r.name}</td>
          <td>${r.diffEquidad.toFixed(2)} USD</td>
          <td><div class="bar-h-container"><div class="bar-h ${cls}" style="width:${pct}%"></div></div></td>
          <td>${txt}</td>
        </tr>`;
    })
    .join("");
  ...
  const tablaMov = (movs) =>
    movs.length === 0
      ...
      <table>
        <thead><tr>
          <th>Fecha</th><th>Persona</th><th>Dir</th><th>Cat</th>
          <th>Monto</th><th>FX</th><th>USD</th><th>Descripción</th>
        </tr></thead>
        <tbody>
          ${movs
            .map((m) => {
              ...
              <tr>
                ...
                <td>${tagDir}</td>
                <td>${tagCat}</td>
                <td>${m.amount_local} ${m.currency}</td>
                <td>${m.fx_to_usd}</td>
  ...
  const content = `
    ...
    <table>
      <thead><tr><th>Hermano</th><th>Diferencia</th><th>Gráfico</th><th></th></tr></thead>
...
app.get("/admin", (req, res) => {
    ...
    movs.length === 0
      ...
      <table>
        <thead>
          <tr>
            <th>Fecha</th><th>Persona</th><th>Dir</th><th>Cat</th>
            <th>Monto</th><th>FX</th><th>USD</th><th>Descripción</th><th></th>
          </tr>
        </thead>
        <tbody>
          ${movs
  ...
  const content = `
    <div class="flash">Panel solo para carga/borrado de movimientos.</div>
    ...
    <form class="form-box" method="POST" action="/admin/movements">
      <div class="form-row">
        ...
        <div class="form-field">
          <label>Persona</label>
          <select name="contributor_id">${optionsContributors}</select>
        </div>
        <div class="form-field">
          <label>Tipo</label>
      ...
      <div class="form-row">
        <div class="form-field">
          <label>Monto local</label>
          <input type="number" step="0.01" name="amount_local">
        </div>
        <div class="form-field">
          <label>Moneda</label>
        ...
        <div class="form-field">
          <label>TC a USD</label>
          <input type="number" step="0.0001" name="fx_to_usd">
        </div>
      </div>
